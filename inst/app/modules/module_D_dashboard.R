      ...
      wellPanel(
        ...
        fluidRow(
          column(4, sliderInput(ns("forest_fontsize"),
                                "Font size (pt)",
                                min = 5, max = 18, value = 10, step = 0.5)),
          column(8, uiOutput(ns("forest_smlab_input")))
        ),
        fluidRow(
          column(6, textInput(ns("forest_label_left"),
                              "Favours left (under x-axis)", value = "")),
          column(6, textInput(ns("forest_label_right"),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$forest_smlab_input <- renderUI({
      ...
      if (is.null(cr) || is.null(cr$net)) {
        return(textInput(ns("forest_smlab"), "Header text", value = ""))
      }
    ...
    forest_opts_r <- reactive({
      ...
      list(
        ...
        fontsize_pt   = as.numeric(input$forest_fontsize  %||% 10),
        smlab         = smlab,
        label_left    = lab_l,
        label_right   = lab_r
      )
    ...
    output$forest_plot_ui <- renderUI({
      ...
      # height = "auto" lets the browser keep the trimmed PNG's aspect
      # ratio. The slider, when non-zero, becomes a manual override
      # measured in CSS pixels.
      slider_h <- input$forest_height %||% 0
      h <- if (is.null(slider_h) || is.na(slider_h) || slider_h <= 0)
             "auto" else paste0(slider_h, "px")
    ...
    output$forest_plot_image <- renderImage({
      ...
      # Strategy: render onto a deliberately oversized canvas so
      # forest.netmeta has plenty of room to lay out, then magick-trim the
      # whitespace and add a 25px breathing border. The final PNG matches
